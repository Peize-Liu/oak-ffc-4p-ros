...
void FFC4PDriver::GrabImgThread(){
	...
	for(auto & queue_node : this->image_queue_){
		...
		if(video_frame != nullptr){
			auto time_host = dai::Clock::now();
			auto time_device = video_frame->getTimestamp();
			printf("%s Current frame time_latency: %ld ms\n",queue_node.topic.c_str(),std::chrono::duration_cast<std::chrono::microseconds>(time_host- time_device).count());
			// cv::imshow(queue_node.topic,video_frame->getCvFrame());
			// cv::waitKey(10);
		
			// printf("%s Current frame sendtime:%f host_time: %f time_latency: %f\n",queue_node.topic,time_host,time_device,time_host -time_device);
		} else {
